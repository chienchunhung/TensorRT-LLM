...
def test_watchdog_reports_local_missing_but_does_not_mark_local_failed(
) -> None:
    health = EPGroupHealth(4)
    reader = FakeCompletionFlagReader(ep_size=4)
...
def test_watchdog_preserves_fifo_order_and_clears_followups_after_timeout(
) -> None:
...
def test_watchdog_from_workspace_reads_phase_specific_offsets() -> None:
    ...
    metainfo[4] = 4
    metainfo[5] = 20
    workspace[ep_rank, 4:16].view(torch.int32).copy_(
        torch.tensor([7, 7, 7], dtype=torch.int32))
    workspace[ep_rank, 20:32].view(torch.int32).copy_(
