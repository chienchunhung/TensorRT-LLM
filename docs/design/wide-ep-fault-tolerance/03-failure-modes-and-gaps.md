...
**Quadrant difficulty.** By difficulty, **Q2 (Mode B1) is the hardest** — no signal to catch; detection requires the explicit kernel-level host watchdog (PR 1a.4) plus MNNVL kernel masking (PR 1a.2, in flight as PR #13404). That's net-new infrastructure, not a fix to existing infrastructure. Q3 (Mode B2) reuses Q1's propagation path so detection is "free"; only the mid-collective cascade adds work (v1). Q4 (Mode B-worst) is rare and out of FT scope.
