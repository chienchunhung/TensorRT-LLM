...
Phase 3 is the *resilience* phase — preventing failures where possible, adapting capacity to load changes, and generally operating above the reactive recover-from-failure loop that Phases 1 and 2 inhabit. It is the lowest-priority phase, not staffed for MVP, and treated at discussion level here; [§8.3](08-implementation-plan.md#83-phase-3-rough-plan) sizes the work at work-track granularity rather than per-PR detail.

The four capabilities below share one design property: they all reuse primitives built in Phases 1 and 2. Phase 3 doesn't introduce a new communication layer or new storage medium; it adds policy on top of the mechanisms already in place.
...
**After Phase 2 lands:**
- 7.1 latency anomaly detection is the natural first item (small scope, high value).
