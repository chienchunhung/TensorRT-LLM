...
# Keep a failed-shutdown transceiver, and therefore its request/session/KV
# roots, alive even if its caller drops the executor.  Removal requires a
# complete physical-drain proof and successful worker shutdown.
...
# Keep this private copy aligned with llmapi.disagg_utils.MIN_GLOBAL_ID. Phase
# 1 accepts only coordinator-issued service requests; local/fallback IDs do not
# provide its required process-lifetime non-reuse contract.
...
class KvCacheTransceiverV2(KvCacheTransceiver):
    ...
    def requires_physical_drain_before_request_release(self) -> bool:
        return getattr(self, "_physical_ownership_enabled", False)
    ...
    def _supports_phase1_physical_ownership(
        ...
        """Validate the disabled-by-default direct context-first canary."""
        ...
        if int(getattr(kv_cache_manager, "max_draft_len", 0) or 0) > 0:
        ...
        py_config = getattr(kv_cache_manager, "kv_cache_manager_py_config", None)
        if len(getattr(py_config, "cache_tiers", ())) > 1:
            unsupported.append("kv_cache_offload_tiers")
        if unsupported:
            raise ValueError(
                f"{_PHYSICAL_OWNERSHIP_ENV}=1 supports only the Phase 1 "
                "direct context-first TP1/PP1/CP1 canary; unsupported: " + ", ".join(unsupported)
            )
        return True
    ...
    def _shutdown_locked(self) -> bool:
        ...
        with self._lifecycle_lock:
            # Closing is sticky even when drain must be retried. No caller may
            # publish another source or destination after this snapshot.
            self._shutdown_started = True
            send_sessions = list(self._send_sessions.values())
            recv_sessions = list(self._recv_sessions.values())
        for session in send_sessions + recv_sessions:
            session.cancel_local()
        unsafe = [
            session.disagg_request_id
            for session in send_sessions + recv_sessions
            if not session.resources_drained()
        ]
        if unsafe:
            # Deregistering NIXL memory or destroying the worker here would
            # invalidate the very ownership evidence that says these accesses
            # may still be live. The caller must retry after drain or terminate
            # the process as the fail-stop containment boundary.
            logger.error(
    ...
    def _validate_phase1_request(
        ...
        *,
        ...
        if not self._physical_ownership_enabled:
            return
        if synchronous:
        ...
        if isinstance(rid, bool) or not isinstance(rid, int) or rid < _MIN_PHASE1_GLOBAL_REQUEST_ID:
            raise ValueError(
                "Phase 1 physical ownership requires a coordinator-issued, "
                "non-reused global disagg_request_id"
            )
    ...
    def _close_failed_sessions(self, sessions: dict, reqs: dict, failed: list):
        retired = []
        for rid in failed:
            # A logical failure is not proof that the backend stopped touching
            # the request's source or destination memory. Keep the request and
            # session roots until their local physical owner has drained.
            requires_drain = self.requires_physical_drain_before_request_release
            if requires_drain and not sessions[rid].resources_drained():
                continue
            reqs[rid].state = LlmRequestState.DISAGG_TRANS_ERROR
            closed = sessions[rid].close()
    ...
    def _respond_and_send_owned(self, req: LlmRequest) -> None:
        """Publish one owned source while holding the lifecycle admission gate."""
        rid = get_unique_rid(req)
        assert rid is not None
        ...
        req.set_kv_cache_transfer_start(tensorrt_llm.bindings.global_steady_clock_now())
        session = self._get_or_create_send_session(req)
        # Retain the request before the first source operation can be
        # authorized.  A metadata/submission exception after that point must
        # not leave an owner whose KV source has lost its request root.
        self._send_reqs[rid] = req
        req.state = LlmRequestState.DISAGG_CONTEXT_TRANS_IN_PROGRESS
        session.send(self._create_kv_slice(req))
        self._finalize_send(req, session)
    ...
    def _prepare_owned_receive(self, req: LlmRequest) -> Optional[tuple[Any, Any]]:
        """Register one destination and publication token without peer I/O."""
        rid = get_unique_rid(req)
        assert rid is not None
        self._raise_if_physical_ownership_fault()
        if getattr(req, _ADMISSION_CANCELLED_ATTR, False):
        ...
        session = self._transfer_worker.create_rx_session(req)
        self._recv_sessions[rid] = session
        # Retain the request before receive() can publish its destination.
        # Post-publication exceptions remain fail-closed until the owner
        # drains, so the destination allocation must keep this root.
        self._recv_reqs[rid] = req
        kv_slice = self._create_kv_slice(req)
        req.py_kv_cache_xfer_bytes = self._slice_num_bytes(kv_slice) * self._kv_size_rank_factor
    ...
    def _raise_if_physical_ownership_fault(self) -> None:
        error = getattr(self._transfer_worker, "physical_ownership_fault", None)
    ...
    def check_context_transfer_status(
        ...
        requires_drain = self.requires_physical_drain_before_request_release
        if requires_drain and terminal:
        ...
        for rid in cancelled:
            if requires_drain and not self._send_sessions[rid].resources_drained():
                continue
    ...
    def check_gen_transfer_status(self, at_least_request_num: Optional[int]):
        ...
        cancelled_reqs = []
        for rid in cancelled:
            if requires_drain and not self._recv_sessions[rid].resources_drained():
                continue
            closed = self._recv_sessions[rid].close()
            if requires_drain and not closed:
                continue
    ...
    def cancel_request(self, req: LlmRequest) -> bool:
        ...
        if rid in self._send_sessions:
            ...
            elif self._send_sessions[rid].close():
                del self._send_reqs[rid]
                del self._send_sessions[rid]
            else:
                has_transferring = True
        ...
        if rid in self._recv_sessions:
            ...
            if self._recv_sessions[rid].has_transferring_tasks():
                has_transferring = True
            elif self._recv_sessions[rid].close():
                del self._recv_reqs[rid]
                del self._recv_sessions[rid]
            else:
                has_transferring = True
    ...
    def _cancel_owned_request(self, req: LlmRequest) -> bool:
        ...
        notifications = []
        with self._lifecycle_lock:
            # The request object is the lifetime root for this one-shot Phase 1
            # admission. Keeping cancellation on that root avoids an unbounded
            # transceiver-wide tombstone set while still fencing every delayed
            # admission of the same request.
